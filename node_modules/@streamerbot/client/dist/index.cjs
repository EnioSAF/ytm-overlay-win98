...
function generateId(prefix = "req") {
  return `sb:client:${prefix}:${Date.now()}-${(0, import_uncrypto.getRandomValues)(new Uint32Array(12))[0]}`;
}
...
function withCustomEventResponse({
  timeout = 1e4,
  addEventListener,
  removeEventListener
}) {
  ...
  const promise = new Promise((resolve, reject) => {
    ...
    signal.addEventListener("abort", () => {
      clearTimeout(timeoutId);
      removeEventListener(
        (listener) => listener.events?.includes("Custom.Event") && listener.callback === handleCustomEvent
      );
    }, { once: true });
  });
...
var StreamerbotClient = class {
  ...
  async updateSupportedEvents() {
    ...
    try {
      ...
      this.supportedEvents = eventsResponse.events;
      ...
      this.supportedEvents = StreamerbotEvents;
    }
  }
  /**
  ...
  async getSupportedEvents() {
    ...
    return this.supportedEvents ?? StreamerbotEvents;
  }
  /**
   ...
   */
  getEventsFromListeners(listeners) {
   ...
   */
  async getSubscriptionsFromListeners(listeners) {
   ...
   */
  async getSubscriptionsFromEventStrings(events) {
    const subscriptions = {};
    ...
    for (const event of events) {
      ...
      for (const item of result) {
        ...
        const set = /* @__PURE__ */ new Set([...subscriptions[source] ?? [], ...eventTypes]);
   ...
   * Get all actions from your connected Streamer.bot instance
   */
  async doAction(action, args, options) {
   ...
   */
  async doActionWithCustomEventResponse(action, args = {}, timeout = 1e4) {
    if (!this.socket || this.socket.readyState !== this.socket.OPEN) {
      throw new Error("WebSocket is not connected");
    }
    const { responseId: sbClientResponse, promise, controller } = withCustomEventResponse({
      timeout,
   ...
   */
  async executeCodeTrigger(triggerName, args, options) {
   ...
   * @returns Promise resolving to the Custom event data
   */
  async executeCodeTriggerWithCustomEventResponse(triggerName, args = {}, timeout = 1e4) {
    if (!this.socket || this.socket.readyState !== this.socket.OPEN) {
      throw new Error("WebSocket is not connected");
    }
    const { responseId: sbClientResponse, promise, controller } = withCustomEventResponse({
